...
public:
  explicit ThreadPool(size_t n) : shutdown_(false) {
    while (n) {
      threads_.emplace_back(worker(*this,n-1));
      n--;
    }
  }
...
private:
  struct worker {
    explicit worker(ThreadPool &pool,int cpu_affinity) : pool_(pool) {
        /*cpu_set_t mask;
        int status;
        CPU_ZERO(&mask);
...
inline Server::Server(const int numPool=CPPHTTPLIB_THREAD_POOL_COUNT)
    : new_task_queue(
          [numPool] { return new ThreadPool(numPool); }),
